  ...
  return (
      <div className="mx-4 my-3 w-52">
        ...
        <div className="p-3 w-full h-56 bg-slate-700 rounded-b-lg text-white text-center ">
          ...
          {text.map((t) => { return (<p className="text-sm">{t}</p>) })}
          <div className="mt-3 flex-center">
            {redirectGitHub ?
              <a className="mx-5" href={redirectGitHub} rel="noreferrer" target="_blank">
                <BsGithub size={27}/>
              </a> : null}
            {redirectGo ?
            <a className="mx-5" href={redirectGo} rel="noreferrer" target="_blank">
              <GoTab size={27}/>
            </a> : null}
          </div>
        </div>
...
const Projects = ({ id }: { id: string }) => {
  return (
    <div id={id} className="mx-12 my-5">
      <h1 className="font-bold text-4xl m-5 flex-center not-phone:justify-start ">Projects</h1>
